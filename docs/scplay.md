...
`scplay` plays a Standard MIDI File through an emulated SC-88, SC-88VL, SC-88Pro, SC-8850, SC-8820,
SC-55mkII or SC-55 and draws the machine's front panel in the terminal: on the 88 family and the SC-55s the
...
| `--rate native \| 32000 \| 44100 \| 48000` | the rate to ask the output device for, and to write `--wav` at.  `native`, the default, is the machine's own — 32 kHz, 66206 Hz on the SC-55mkII and 64000 Hz on the SC-55.  The machine always renders at its own rate; what this picks is what it is converted to |
...
The firmware takes about six seconds of machine time to boot, 4.75 s on the SC-55mkII and 1.26 s on
the SC-55 — and on the SC-55mkII twice that the first time, because a machine whose settings memory has
never been written comes up a semitone flat and has to be booted again on what the first boot wrote.  scplay runs
...
The SC-55mkII's and the SC-55's is that list less the buttons they have not got: they have no map buttons, no USER INST, no
...
On the 88 family and the SC-55s the panel is drawn from the LCD controller's memory.  The text
...
lens die appears as EFX.  The SC-55mkII's and the SC-55's row is their own three, ALL, MUTE and
STANDBY, the last of them the machine's own lamp: its POWER key is a position in the panel matrix, and the firmware answers
...
kHz, the SC-55mkII's 66206 Hz and the SC-55's 64000 Hz — or 32000, 44100 or 48000.  A device that will not open at the
