...
static const rom_set_t ROM_SETS[] =
{
	...
	{ "sc8820", SCEMU_MODEL_SC8820, { "roland-r01561945.ic5", 0x200000 },
	  { { "roland-r01891445-823.ic7", 0x1000000 }, { "roland-r02121512-541.ic8", 0x800000 } }, 2,
	  { "roland-r02015367.ic1", 0x10000 }, { NULL, 0 } },
	...
	{ "sc55", SCEMU_MODEL_SC55, { "roland_r15209363.ic23", 0x40000 },
	  { { "roland-gss.a_r15209276.ic28", 0x100000 }, { "roland-gss.b_r15209277.ic27", 0x100000 },
	    { "roland-gss.c_r15209281.ic26", 0x100000 } }, 3,
	  { "roland_r15199778_6435328a97f.ic30", 0x8000 }, { NULL, 0 } },
};
...
static int write_raw(const char *path, const int32_t *stereo, size_t frames)
{
	FILE *fp = fopen(path, "wb");
	if (!fp)
		return 0;
	fwrite(stereo, sizeof(int32_t), frames * 2, fp);
	fclose(fp);
	return 1;
}
...
{
	fprintf(stderr, "usage: %s <sc88|sc88vl|sc88pro|sc8850|sc8820|sc55mk2|sc55> <romdir> <out.wav> [--midi file.mid] [--seconds N] [--tail N] [--state boot.state] [--map sc55|sc88|sc88pro|sc8850] [--midi-rate BAUD] [--computer midi|pc1|pc2|usb] [--raw words.bin] [--no-jit]\n", argv0);
}
...
int main(int argc, char **argv)
{
	...
	if (!computer_given && (set->model == SCEMU_MODEL_SC8850 || set->model == SCEMU_MODEL_SC8820))
		computer = SCEMU_COMPUTER_MAC;
	scemu_set_computer_switch(m, computer);
	const bool usb = computer == SCEMU_COMPUTER_MAC;
	const int midi_ports = set->model == SCEMU_MODEL_SC8850 ? (usb ? 4 : 2)
	                     : set->model == SCEMU_MODEL_SC8820 ? (usb ? 2 : 1) : 2;
	...
	if (raw_path && !write_raw(raw_path, buf, frames))
	{
