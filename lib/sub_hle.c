...
typedef struct exclusive_block { uint8_t model; uint8_t address; uint8_t code; bool paged; } exclusive_block_t;
...
static void send_sysex(sub_hle_t *sub, int src, const sub_source_t *s)
{
	...
	if (size >= 8 && x[0] == 0x41 && (x[2] == 0x42 || x[2] == 0x45) && (x[3] == 0x12 || x[3] == 0x11))
	{
		const exclusive_block_t *block = NULL;
		for (size_t i = 0; i < sizeof(BLOCKS) / sizeof(BLOCKS[0]); i++)
			if (BLOCKS[i].model == x[2] && BLOCKS[i].address == x[4] && (BLOCKS[i].paged || (x[5] & 0xf0) == 0))
				block = &BLOCKS[i];
		if (!block)
			return;
